...
new Vue({el:"#app",data:{pass:"",phone:"",remember:!1},methods:{submitForm:function(e){$.ajax({url:e.target.action,method:"GET",data:{phone:this.phone,pass:this.pass,remember:this.remember},success:function(){alert("All good and data send")},error:function(){alert("Error occuried")},complete:function(){e.target.reset()}}),this.phone="",this.pass="",this.remember=!1}},computed:{isValidForm:function(){return this.isValidPhone&&this.isValidPass},isValidPhone:function(){if(0<this.phone.length)return!!/^\d{1}\s\(\d{3}\)\s\d{2}-\d{2}-\d{2}$/.test(this.phone.trim())},isValidPass:function(){return 5<this.pass.length}},mounted:function(){var e=document.getElementsByClassName("blured-image")[0],t=document.getElementById("app").offsetHeight,s=document.getElementById("app").offsetWidth;e.setAttribute("style","height:"+t+"px;width:"+s+"px;")}});